...
![](docs/nrd_denoiser.png)

This example is demonstrating [NRD, "NVIDIA Realtime Denoiser",
](https://github.com/NVIDIAGameWorks/RayTracingDenoiser) in a simple path
tracer rendering a glTF scene . NRD is a spatio-temporal post-processing that
removes noise from Monte-Carlo based path tracers. NRD is not just a single
...
NRD itself provides a comprehensive sample at [NRD Sample
https://github.com/NVIDIAGameWorks/NRDSample](https://github.com/NVIDIAGameWorks/NRDSample),
...
alongside showing a simple pathtracer using NRD to denoise the rendered image.
...
In our example, we are testing ReBLUR and ReLAX.
...
# Make our project dependent on NRD
...
these shader binaries. Refer to `externals/nrd/CMakeLists.txt` for a list of
...
NRD itself is platform agnotic and thus can be used with DirectX and Vulkan.
...
* As 'white box' where the application is asked to do everything: allocate 
  needed resources AND compile NRD's shaders
* 'black-box library': the application creates all resources, but the shaders
  will be handed to the application as precompiled binary blobs (SPIR-V, DXIL etc)
* via NRD's integration layer. NRD SDK provides an integration wrapper based
  on NRI (NVIDIA Rendering Interface)
...
requires. NRD is naturally written in HLSI, but can be instructed to emit GLSL
compatible syntax. We achieve this by defining `NRD_GLSL 1` at the top of
`nrd.glsl` before including `nrd.hlsi`

NRD build is using a SPIR-V-enabled DirectX shader compiler 'DXC' to
compile its HLSL shaders into SPIR-V that Vulkan can digest. 'dxc' typically
comes with the Vulkan SDK. We recommend installing the latest Vulkan SDK for
that. Another option is to download
...
wrapper class, `NRDWrapper`, contains all the neccessary code for that. Since
NRD is mostly graphics API agnostic (supporting D3D and Vulkan), it describes
...
executing a series of compute shader invocations with proper pipeline barries
inbetween the passs. Code comments in NRDWrapper.cpp/.hpp describe the details
of its API and implementation. Additionally, one can find a lot of information
in the NRD headers themselves. Notably, `NRDDesc.h` and `NRD.hlsi` are worth
...
NRDWrapper creates all NRD-internal textures on its own, the the exception of
the "user texture pool". This is a pool of textures that both, NRD and the application
operate on. NRD calls these 'resources'. The user texture pool may be sparsely
populated, depending on the used denoiser. Consult NRDDescs.h to see which denoisers
require which type of resource (i.e. texture) and what format that texture needs
to have.
...
2. The first step for each ray is to find the first non-mirrored surface, the _primary hit_.
   At the primary hit we record all material properties, normals etc that NRD needs and write
...
3. Get the direct light contributions. For this, we loop through
   ...
   light emssision here. The direct light contributions are not noisy and thus
...
4. Probe the environment map lighting at the primary hit. The environment map
   has been preprocessed to provide a PDF and a randomzied sampling pattern
   ...
   weighted; see [MIS weighting](#MIS-weighting)

5. From the hit position we perform path-tracing to collect the incoming indirect light -
   once along the material's diffuse BSDF, once along its specular BSDF.
   For this tracing we use the pathtrace.rchit shader. At each hit position we do the following:
    - If the ray missed all geometry, we hit the environment map. Return its light contribution,
      weigh it with [MIS weight](#MIS-weighting) and report the end of this ray via `payload.hitT = NRD_INF`
    ...
    - In case of an absorbtion event, return `payload.hitT = NRD_INF` to end the path here
    ...
    - Repeat the paths tracing until the ray dies and take the first bounce path length.
...
* `VIEWZ`: This is the linear depth of the hit position in camera space. With the hit position, you can do: `float z = abs((worldToView * hitPosition).z)` to get the expected value.
* it also takes a couple of optional inputs that this sample does not provide for simplicity's sake

ViewZ![img](docs/viewz.png)
...
NRD can only work, if it is provided with the right data. NRD's [README.md](external/nrd/README.md)
talks about this is much detail. Below we will mention a few critical points to
get right.
...
channel, but for best denoising quality, its best to separate out these two
noisy signals at the primary hit (or PSR as shown later)

![img](docs/NRD_sketch.png)

Splitting the pathtraced image into a diffuse and specular image requries
separately following the diffuse and specular material's BSDF at the position
of the first non-mirrored hit. This sample chooses to produce each signals's
image at full resolution, which also means we're doubling the amount of rays
per pixel. Other approaches may used as well:

  * a dithered pattern where every other pixel position follows either diffuse or specular path.
  * a probabilistic approach where following either diffuse or specular path is a random choice
...
fashion. This is expressed in `shaders/nrd.rgen` marked with `#DIFFUSE` and
`#SPECULAR`. The pathtracing code for gathering the indirect light contributions
...
NRD is asking a scalar 'hit distance' to be provided. This is typically the
...
requires special handling (refer to NRD's [README.md](external/rnd/README.md)).
...
following the mirorred ray until it hits something that is not a mirror - the
...
virtual world space, like Normal, Roughness and ViewZ. This helps the denoiser
denoising reflected objects much better.
...
coming from textures at the primary hit surface. You certainly don't want the
...
The 'composition.comp' shader will later modulate the denoised diffuse and
specular images with the inverse of the demodulation term and recombine all
parts of the image (specular, diffuse, direct light) into a single one.

### MIS weighting

The implemented pathtracer is using importance sampling to approximate the
integrals of incoming light and material's response in a single frame, while
the denoiser is doing integration over time. Care has to be taken when the
pathtracer samples the same function multiple times, but uses different PDFs
...
light contribution ast that point. It then follows the Material's BSDF to probe
...
direct ligthing. Diffuse and specular are added only if there
was a hit and albedo is extracted from the base color - metalness information.
