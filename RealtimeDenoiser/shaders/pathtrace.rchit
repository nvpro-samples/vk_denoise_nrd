...
ShadingResult shading(in PbrMaterial pbrMat, in HitState hit)
{
  ...
  const bool nextEventValid = (dot(dirToLight, pbrMat.N) > 0.0f) && lightPdf > 0.0f;
  ...
  if(nextEventValid)
  {
    ...
    if(evalData.pdf > 0.0)
    {
      ...
      const float mis_weight = powerHeuristic(lightPdf, evalData.pdf);
  ...
  // Sample BSDF to suggest a follow-up ray for more indirec lighting
  {
