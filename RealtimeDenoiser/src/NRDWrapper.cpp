...
static const VkFormat g_NRDFormatToVkFormat[size_t(nrd::Format::MAX_NUM)] = {
    VK_FORMAT_R8_UNORM,
    VK_FORMAT_R8_SNORM,
    VK_FORMAT_R8_UINT,
    VK_FORMAT_R8_SINT,
...
static const VkDescriptorType g_NRDDescriptorTypeToVulkan[size_t(nrd::DescriptorType::MAX_NUM)] = {
...
static const VkFilter g_NRDtoVkFilter[size_t(nrd::Sampler::MAX_NUM)] = {VK_FILTER_NEAREST, VK_FILTER_LINEAR};
...
VkFormat NRDWrapper::getNormalRoughnessFormat()
{
  ...
  switch(nrd::GetLibraryDesc().normalEncoding)
  {
    ...
    case nrd::NormalEncoding::RGBA16_SNORM:
      ...
      // Docs say, this one may also be "FP formats"
...
void NRDWrapper::createPipelines()
{
  ...
  for(uint32_t p = 0; p < iDesc.pipelinesNum; ++p)
  {
    ...
    if(pDesc.hasConstantData)
    {
      ...
      constantBinding.pImmutableSamplers = NULL;
    }
    ...
    for(uint32_t r = 0; r < pDesc.resourceRangesNum; ++r)
    {
      ...
      for(uint32_t b = 0; b < range.descriptorsNum; ++b)
      {
        ...
        resourceBindings.pImmutableSamplers = NULL;
    ...
    LOGI("Pipeline is using %d bindings\n", numBindings);
