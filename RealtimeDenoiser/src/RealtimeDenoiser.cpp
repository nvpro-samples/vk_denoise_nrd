...
public:
  ...
  void onAttach(nvvkhl::Application* app) override
  {
    ...
    m_sceneVk = std::make_unique<SceneVk>(m_device, m_app->getPhysicalDevice(), m_alloc.get());    // GLTF Scene buffers
    m_sceneRtx = std::make_unique<SceneRtx>(m_device, m_app->getPhysicalDevice(), m_alloc.get());  // GLTF Scene BLAS/TLAS
    ...
    m_hdrEnv     = std::make_unique<HdrEnv>(m_device, m_app->getPhysicalDevice(), m_alloc.get());
    ...
    {  // Setting menu
      ...
      if(ImGui::CollapsingHeader("Denoiser", ImGuiTreeNodeFlags_DefaultOpen))
      {
        ...
        if(ImGui::BeginTable("thumbnails", 2))
        {
          ...
          showBuffer("Diffuse Radiance(YCoCg)", eGBufDiffRadianceHitDist);
          ImGui::TableNextColumn();
          showBuffer("Specular Radiance(YCoCg)", eGBufSpecRadianceHitDist);
          ImGui::TableNextRow();
          ImGui::TableNextColumn();
    ...
    {
      {
        ...
        //memcpy(m_nrdSettings.rectSizePrev, m_nrdSettings.rectSize, sizeof(nrd::CommonSettings::rectSizePrev));
      ...
      {
        case NRD_REBLUR: {
          // m_reblurSettings.enableAntiFirefly = true;
        ...
        case NRD_RELAX: {
          ...
          // m_relaxSettings.enableAntiFirefly = true;
...
private:
  ...
  {
    static auto depth_format = VK_FORMAT_UNDEFINED;  //nvvk::findDepthFormat(m_app->getPhysicalDevice());  // Not all depth are supported
  ...
  void createRtxPipeline()
  {
    ...
    // Raygen
    stage.module = nvvk::createShaderModule(m_device, /*pathtrace_rgen*/ nrd_rgen, sizeof(/*pathtrace_rgen*/ nrd_rgen));
  ...
  {
    m_hdrEnv = std::make_unique<HdrEnv>(m_app->getDevice(), m_app->getPhysicalDevice(), m_alloc.get());
  ...
  void createCompositionPipeline()
  {
    ...
    m_dutil->setObjectName(m_compositionDescSetlayout, "Composition");
    ...
    m_dutil->setObjectName(m_compositionPipeline, "Composition");
  ...
  void createTaaPipeline()
  {
    ...
    m_dutil->setObjectName(m_compositionDescSetlayout, "Taa");
    ...
    m_dutil->setObjectName(m_taaPipeline, "Taa");
...
auto main(int /*argc*/, char** /*argv*/) -> int
{
  ...
  auto nrd_denoiser = std::make_shared<nvvkhl::NRDEngine>();
